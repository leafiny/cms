...
/**
 * Class CartRule_Helper_Cart_Rule
 */
class CartRule_Helper_Cart_Rule extends Core_Helper
{
    /**
    ...
    public function getCartRules(?Leafiny_Object $sale = null, ?array $types = null): array
    {
        ...
        try {
            ...
            /** @var CartRule_Model_Cart_Rule $ruleModel */
    ...
    public function getShippingDiscountRate(?Leafiny_Object $sale = null): float
    {
        ...
        try {
            ...
            $rules = $this->getCartRules($sale, [CartRule_Model_Cart_Rule::TYPE_PERCENT_SHIPPING]);
    ...
    public function addFreeGift(?int $saleId = null): void
    {
        try {
            ...
            $sale  = $this->getSale($saleId);
            $rules = $this->getCartRules($sale, [CartRule_Model_Cart_Rule::TYPE_FREE_GIFT]);
        ...
        try {
            ...
            foreach ($rules as $rule) {
                ...
                foreach ($items as $item) {
                    ...
                    if ($rule->getData('type') === CartRule_Model_Cart_Rule::TYPE_PERCENT_SUBTOTAL) {
                        $inclTaxDiscount = $item->getData('incl_tax_unit') * $discount / 100;
                        $exclTaxDiscount = $item->getData('excl_tax_unit') * $discount / 100;
                    }
                    if ($rule->getData('type') === CartRule_Model_Cart_Rule::TYPE_AMOUNT_PER_PRODUCT) {
    ...
    public function getCartRuleAllowedTypes(): array
    {
        return array_merge_recursive(
            [
                'discount' => [
                    CartRule_Model_Cart_Rule::TYPE_PERCENT_SUBTOTAL   => 'Subtotal Discount (%)',
                    CartRule_Model_Cart_Rule::TYPE_PERCENT_SHIPPING   => 'Shipping Discount (%)',
                    CartRule_Model_Cart_Rule::TYPE_AMOUNT_PER_PRODUCT => 'Discount per product (amount)',
                ],
                'option' => [
                    CartRule_Model_Cart_Rule::TYPE_FREE_GIFT => 'Free Gift',
                ],
            ],
            $this->getCustom('cart_rule_alCartRulelowed_types') ?? []
        );
    }
    ...
    public function getCartRuleTypeCartDiscount(): array
    {
        return array_merge(
            [
                CartRule_Model_Cart_Rule::TYPE_PERCENT_SUBTOTAL,
                CartRule_Model_Cart_Rule::TYPE_AMOUNT_PER_PRODUCT,
            ],
